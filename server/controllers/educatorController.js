...
export const addCourse =async(req,res)=>{
    try {
      ...
      if(!imageFile){
       return  res.json({success:false,message:"ThumbNail not recieved"})
      }
...
// Get Enrollrd Students Data with Purchase Data

export const getEnrolledStudentData=async(req,res)=>{
   try {
      const educatr= req.auth.userId
      const courses = await Course.find({educator});
      const courseIds = courses.map(course => course._id);
      ...
      const enrolledStudents = purchase.map(purchase =>({
         ...
         courseTitle:purchase.courseId.createAt
      }))
