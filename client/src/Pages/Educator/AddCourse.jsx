    ...
    <div className='h-screen flex flex-col items-start justify-between md:p-8 md:pb-0 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
          <input onChange={(e)=>setCourseTitle(e.target.value)} value={courseTitle} className='outline-none md:py-2.5 py-2 px-3 rounded border border-gray-500' type="text" required />
        </div>
        <div className='flex flex-col gap-1'>
         ...
         <div className='flex items-center justify-between flex-wrap'>
          <div className='flex-flex-col gap-1'>
   <p> Course Price</p>
   <input onChange={(e)=>setCoursePrice(e.target.value)} value={coursePrice} className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500'  type="number" required />
          </div>
          <div className='flex md:flex-row flex-col items-center gap-3'>
           ...
           <div className='flex flex-col gap-1'>
            <p>Discount %</p>
            <input onChange={(e)=>setDiscount(e.target.value)} value={discount} placeholder='0' min={0} max={100} className='outline-none md:py-2.5 py-2 w-28 px-3 rounded border border-gray-500' type="number" required />
           </div>
    ...
    <img src={assets.dropdown_icon} onClick={()=>handleChapter('toggle',chapter.chapterId)} width={14} className={`mr-2 cursor-pointer transition-all ${chapter.collapsed && "-rotate-90"}`} alt="" />
           ...
           <button className='bg-black rounded my-4 w-max py-2.5 text-white px-8' type='submit'>ADD</button>
      </form>
    </div>
  )
}
